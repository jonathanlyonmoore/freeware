   ...
   png_textp text,int ii)
{
  ...
  {
    (void) ThrowMagickException(&image->exception,GetMagickModule(),CoderError,
      "UnableToCopyProfile","`%s'","invalid profile length");
    return (MagickFalse);
  }
  ...
  for (i=0; i < (long) nibbles; i++)
  {
    while (*sp < '0' || (*sp > '9' && *sp < 'a') || *sp > 'f')
    {
      if (*sp == '\0')
        {
          (void) ThrowMagickException(&image->exception,GetMagickModule(),
            CoderError,"UnableToCopyProfile","`%s'","ran out of data");
        ...
        if (memcmp(type,mng_MHDR,4) == 0)
          {
            ...
            if (mng_info->ticks_per_second == 0)
              ...
              default_frame_delay=image->ticks_per_second/mng_info->ticks_per_second;
                ...
                if ((p-chunk) < (long) (length-4))
                  {
                    ...
                    if (change_delay)
                      {
                        frame_delay=(image->ticks_per_second*(mng_get_long(p))/
                    ...
                    if (change_timeout)
                      {
                        frame_timeout=
                            (image->ticks_per_second*(mng_get_long(p))/mng_info->ticks_per_second);
                        if (change_delay == 2)
...
#define QM Quantum
                ...
                if (image->matte != MagickFalse)
                   SetImageBackgroundColor(large_image);
                else
                  {
  ...
  if (mng_info->ticks_per_second)
    final_delay=Max(image->ticks_per_second,1)*final_delay/mng_info->ticks_per_second;
  else
    image->start_loop=MagickTrue;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%  RegisterPNGImage() adds attributes for the PNG image format to
   ...
   *profile_description, unsigned char *profile_data, png_uint_32 length)
{
   ...
   *dp++='\n';
   (void) FormatMagickString(dp,allocated_length-strlen(text[0].text),"%8lu ",length);
   dp+=8;
   for (i=0; i < (long) length; i++)
   {
     if (i%36 == 0)
  ...
  */
  ...
  if (mng_info->write_png8)
  ...
  else
    {
      ...
      if ((mng_info->optimize == 0) && (matte != MagickFalse))
        ping_info->color_type=(png_byte) PNG_COLOR_TYPE_RGB_ALPHA;
    ...
    if (profile != (StringInfo *) NULL)
      {
        ...
        if ((LocaleCompare(name,"ICC") == 0) ||
          ...
          png_set_iCCP(ping,ping_info,(png_charp) name,0,(png_charp)
            profile->datum,(png_uint_32) profile->length);
        else
#endif
          png_write_raw_profile(image_info,ping,ping_info,(unsigned char *)
            name,(unsigned char *) name,(unsigned char *) profile->datum,
            (png_uint_32) profile->length);
      }
    if (logging != MagickFalse)
      (void) LogMagickEvent(CoderEvent,GetMagickModule(),
 ...
 * 1.0.5a is leaky */
        if (ping_info->num_text == 0)
          {
            ping_info->text=(png_text *)
              AcquireMagickMemory(256*sizeof(png_text));
  ...
  if (write_mng)
    ...
    defined(PNG_MNG_FEATURES_SUPPORTED)
      ...
      for (next_image=image; next_image != (Image *) NULL; )
      {
        ...
        if (final_delay != initial_delay || final_delay > next_image->ticks_per_second)
     ...
     if (mng_info->need_fram == MagickFalse)
       {
         ...
         */
         ...
         if (final_delay != 0)
           mng_info->ticks_per_second=image->ticks_per_second/final_delay;
         if (final_delay > 50)
         ...
         if (need_defi && final_delay > 2 && (final_delay != 4) &&
            ...
            (final_delay != 25) && (final_delay != 50) && (final_delay != image->ticks_per_second))
     ...
     if (option != (const char *) NULL)
       {
         ...
         */
         ...
         LogPNGChunk((int) logging,mng_nEED,length);
         length+=4;
     ...
     if ((GetPreviousImageInList(image) == (Image *) NULL) &&
         ...
         (image->iterations != 1))
       {
         ...
         */
         ...
         PNGLong(chunk+6,(png_uint_32) (mng_info->ticks_per_second*final_delay/Max(image->ticks_per_second,1)));
         ...
         if (logging != MagickFalse)
           {
             (void) LogMagickEvent(CoderEvent,GetMagickModule(),
               "     TERM delay: %lu",
               (png_uint_32) (mng_info->ticks_per_second*final_delay/Max(image->ticks_per_second,1)));
             if (image->iterations == 0)
               (void) LogMagickEvent(CoderEvent,GetMagickModule(),
       ...
       else
         {
           ...
           */
           ...
           PNGLong(chunk+10,(png_uint_32)
             ((mng_info->ticks_per_second*image->delay)/Max(image->ticks_per_second,1)));
           (void) WriteBlob(image,14,chunk);
           (void) WriteBlobMSBULong(image,crc32(0,chunk,14));
