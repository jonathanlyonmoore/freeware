...
#define XMLProfile  "begin_xml_packet:"
    ...
    if (LocaleNCompare(PhotoshopProfile,command,strlen(PhotoshopProfile)) == 0)
      {
        /*
          Read image profile.
        */
        count=(ssize_t) sscanf(command,PhotoshopProfile " %lu",&length);
        if (count != 1)
          continue;
    ...
    if (LocaleNCompare(XMLProfile,command,strlen(XMLProfile)) == 0)
      {
        /*
          Read image profile.
        */
        count=(ssize_t) sscanf(command,XMLProfile " %lu",&length);
        if (count != 1)
          continue;
  ...
  *read_info->magick='\0';
  ...
  do
  {
    ...
    if (columns != 0)
      image->magick_columns=columns;
    if (rows != 0)
      image->magick_rows=rows;
