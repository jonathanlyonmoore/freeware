...
{ \
  ...
  while (p < (blocks+length))
  {
    if (LocaleNCompare((char *) p,"8BIM",4) != 0)
      break;
    PushLongPixel(sans,p);
    PushShortPixel(id,p);
    PushShortPixel(sans,p);
    PushLongPixel(count,p);
    switch (id)
    {
      case 0x03ed:
      {
      ...
      if (psd_info.mode == DuotoneMode)
        {
          ...
          */
          data=(unsigned char *) AcquireMagickMemory(length);
      ...
      else
        {
          ...
          */
          if (AllocateImageColormap(image,length/3) == MagickFalse)
            ThrowReaderException(ResourceLimitError,"MemoryAllocationFailed");
  ...
  if (length != 0)
    {
      ...
      */
      ...
      blocks=(unsigned char *) AcquireMagickMemory(length);
      ...
      if (size == 0)
      ...
      else
        {
          ...
          layer_info=(LayerInfo *)
            AcquireMagickMemory((size_t) number_layers*sizeof(*layer_info));
      ...
      if (image->debug != MagickFalse)
          ...
          combinedlength += 12 + length;  /* sig, key, length + the actual length*/
            ...
            */
            ...
            SetImageBackgroundColor(layer_info[i].image);
            ...
            */
            FormatMagickString(s,MaxTextExtent,"%ld", layer_info[i].page.x );
            (void) SetImageAttribute(layer_info[i].image,"[layer-xpos]",s);
            FormatMagickString(s,MaxTextExtent,"%ld", layer_info[i].page.y );
            (void) SetImageAttribute(layer_info[i].image,"[layer-ypos]",s);
            FormatMagickString(s,MaxTextExtent,"%lu",(unsigned long)
            ...
            for (j=0; j < (long) layer_info[i].channels; j++)
            {
              ...
              */
              ...
              for (y=0; y < (long) layer_info[i].image->rows; y++)
              {
                ...
                {
                  if (packet_size == 1)
                    pixel=ScaleCharToQuantum((unsigned char)
                      ...
                      if (layer_info[i].image->storage_class == PseudoClass)
                        {
                          ...
                          q->red=layer_info[i].image->colormap[*indexes].red;
                          q->green=layer_info[i].image->colormap[*indexes].green;
                          q->blue=layer_info[i].image->colormap[*indexes].blue;
                        }
                      break;
                    }
            ...
            if (layer_info[i].opacity != OpaqueOpacity)
              {
                ...
                */
                for (y=0; y < (long) layer_info[i].image->rows; y++)
                {
                  ...
                  for (x=0; x < (long) layer_info[i].image->columns; x++)
                  {
                    q->opacity=QuantumRange-(Quantum) (QuantumScale*(q->opacity*
                      layer_info[i].opacity));
                    q++;
                  }
                  if (SyncImagePixels(layer_info[i].image) == MagickFalse)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%  RegisterPSDImage() adds attributes for the PSD image format to
...
static void WriteImageChannels(const ImageInfo *image_info,Image *image,
  Image *tmp_image,unsigned char *pixels)
{
  /*
...
static MagickBooleanType WritePSDImage(const ImageInfo *image_info,Image *image)
{
  ...
  */
  ...
  pixels=(unsigned char *)
    AcquireMagickMemory(packet_size*image->columns*sizeof(PixelPacket));
  ...
  if (profile == (StringInfo *) NULL)
    ...
    {
      (void) WriteBlobMSBLong( image, profile->length );
...
compute_layer_info:
  ...
  else
  {
    ...
    while ( tmp_image != NULL ) {
      ...
      theAttr=(ImageAttribute *) GetImageAttribute(tmp_image,"label");
      if (theAttr) {
    ...
    tmp_image = base_image;
    while ( tmp_image != NULL ) {
      WriteImageChannels(image_info,image,tmp_image,pixels );
  ...
  WriteImageChannels(image_info,image,image,pixels);
