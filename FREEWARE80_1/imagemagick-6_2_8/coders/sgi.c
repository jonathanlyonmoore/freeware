  ...
  */
  ...
  do
  {
    ...
    */
    ...
    (void) ReadBlob(image,sizeof(iris_info.name),iris_info.name);
    iris_info.name[sizeof(iris_info.name)-1]='\0';
    iris_info.pixel_format=ReadBlobMSBLong(image);
    count=ReadBlob(image,sizeof(iris_info.filler),iris_info.filler);
    image->columns=iris_info.columns;
    image->rows=iris_info.rows;
    image->depth=Min(iris_info.depth,QuantumDepth);
    if (iris_info.pixel_format == 0)
      image->depth=Min(8*iris_info.bytes_per_pixel,QuantumDepth);
    ...
    if ((int) iris_info.storage != 0x01)
    ...
    else
      {
        ...
        */
        ...
        max_packets=(unsigned char *) AcquireMagickMemory(4*(size_t)
        ...
        */
        ...
        for (y=0; ((y < (long) iris_info.rows) && (data_order == MagickFalse)); y++)
          for (z=0; ((z < (long) iris_info.depth) && (data_order == MagickFalse)); z++)
          {
  ...
  */
  ...
  do
  {
    ...
    */
    ...
    (void) WriteBlob(image,sizeof(iris_info.name),iris_info.name);
