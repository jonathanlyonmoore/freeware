  ...
  if (poly_rect->count > 0)
    {
      ...
      ddata->clip_path_id++;
      DrawPushDefs(WmfDrawingWand);
      (void) FormatMagickString(clip_path_id,MaxTextExtent,"clip_%lu",
        ddata->clip_path_id);
      DrawPushClipPath(WmfDrawingWand,clip_path_id);
      (void) PushDrawingWand(WmfDrawingWand);
      for (i = 0; i < poly_rect->count; i++)
        {
          DrawRectangle(WmfDrawingWand,
      ...
      (void) PushDrawingWand(WmfDrawingWand);
      (void) DrawSetClipPath(WmfDrawingWand,clip_path_id);
...
static void util_set_pen(wmfAPI * API, wmfDC * dc)
{
  ...
  DrawSetStrokeWidth(WmfDrawingWand, Max(0, pen_width));
