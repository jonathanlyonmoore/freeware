  ...
  Image *images)
{
  ...
  if (*image_info->filename != '\0')
    resource_info.window_id=(char *) image_info->filename;
  ...
  for (i=0; (state & ExitState) == 0; i++)
  {
    ...
    if (*image_info->filename == '\0')
      ...
      {
        (void) XDisplayBackgroundImage(display,&resource_info,image);
        break;
      }
  }
        ...
        switch (ColorEditCommands[id])
        {
          ...
          case ColorEditFuzzCommand:
          {
            ...
            */
            ...
            if (entry != 5)
              {
                (*image)->fuzz=StringToDouble(FuzzMenu[entry],QuantumRange);
                break;
              }
            ...
            (*image)->fuzz=StringToDouble(fuzz,QuantumRange);
            break;
          }
      ...
      if (mozilla_window != (Window) NULL)
        {
          ...
          */
          ...
          (void) FormatMagickString(command,MaxTextExtent,
            "openURL(%s,new-window,noraise)",url);
  ...
  for (i=0; i < (long) number_files; i++)
  {
    ...
    *read_info->magick='\0';
    ...
    if (next_image != (Image *) NULL)
      {
        (void) DeleteImageAttribute(next_image,"label");
  ...
  XResourceInfo *resource_info,Image *image)
{
  ...
  */
  ...
  delay=1000*image->delay/Max(image->ticks_per_second,1);
...
#define MagickTitle  "Commands"
  ...
  */
  if (windows->image.id != (Window) NULL)
    {
      (void) CloneString(&windows->image.name,"\0");
      (void) CloneString(&windows->image.icon_name,"\0");
    }
  XGetWindowInfo(display,visual_info,map_info,pixel,font_info,
  ...
  if ((resource_info->title != (char *) NULL) && !(*state & MontageImageState))
    {
      (void) CloneString(&windows->image.name,InterpretImageAttributes(
        resource_info->image_info,display_image,resource_info->title));
      (void) CloneString(&windows->image.icon_name,InterpretImageAttributes(
        resource_info->image_info,display_image,resource_info->title));
    }
  else
    {
  ...
  */
  if (windows->widget.id != (Window) NULL)
    (void) CloneString(&windows->widget.name,"\0");
  XGetWindowInfo(display,visual_info,map_info,pixel,font_info,
  ...
  windows->widget.geometry=XGetResourceClass(resource_info->resource_database,
    resource_name,"geometry",(char *) NULL);
  windows->widget.name=AcquireString((char *) NULL);
  ...
  */
  if (windows->popup.id != (Window) NULL)
    (void) CloneString(&windows->popup.name,"\0");
  XGetWindowInfo(display,visual_info,map_info,pixel,font_info,
    resource_info,&windows->popup);
  windows->popup.name=AcquireString((char *) NULL);
  ...
  */
  if (windows->magnify.id != (Window) NULL)
    (void) CloneString(&windows->magnify.name,"\0");
  XGetWindowInfo(display,visual_info,map_info,pixel,font_info,
  ...
  windows->magnify.geometry=XGetResourceClass(resource_info->resource_database,
    resource_name,"geometry",(char *) NULL);
  windows->magnify.name=AcquireString((char *) NULL);
  ...
  */
  ...
  /*
    Respond to events.
  */
  delay=display_image->delay/Max(display_image->ticks_per_second,1);
                ...
                *state|=NextImageState | ExitState;
              else
                {
                  ...
                  */
                  ...
                  delay=display_image->delay/
                    Max(display_image->ticks_per_second,1);
                  timer=time((time_t *) NULL)+(delay == 0 ? 1 : delay)+1;
                }
            }
