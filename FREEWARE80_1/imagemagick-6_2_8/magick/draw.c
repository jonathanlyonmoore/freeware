  ...
  const Image *source,const AffineMatrix *affine)
{
  ...
  */
  ...
  edge.x1=Max(min.x,0.0);
  edge.y1=Max(min.y,0.0);
  edge.x2=Min(max.x,(double) image->columns-1.0);
  edge.y2=Min(max.y,(double) image->rows-1.0);
  ...
  for (y=(long) (edge.y1+0.5); y < (long) (edge.y2+0.5); y++)
  {
    ...
    q=GetCacheView(image_view,(long) (inverse_edge.x1+0.5),y,(unsigned long)
      (inverse_edge.x2-inverse_edge.x1+0.5),1);
    ...
    for (x=(long) (inverse_edge.x1+0.5); x < (long) (inverse_edge.x2+0.5); x++)
    {
      ...
      point.y=(double) x*inverse_affine.rx+y*inverse_affine.sy+
        inverse_affine.ty;
      pixel=InterpolateColor(source,source_view,point.x,point.y,
        &image->exception);
  ...
  const DrawInfo *draw_info,const char *name)
{
  ...
  (void) FormatMagickString(clip_path,MaxTextExtent,"[%s]",name);
  ...
  SetImageBackgroundColor(image->clip_mask);
  if (image->debug != MagickFalse)
    (void) LogMagickEvent(DrawEvent,GetMagickModule(),"\nbegin clip-path %s",
      draw_info->clip_path);
  clone_info=CloneDrawInfo((ImageInfo *) NULL,draw_info);
  ...
  const PrimitiveInfo *primitive_info,Image *image)
{
  ...
  for (i=1; i < (long) number_vertices; i++)
  {
    ...
    maximum_length=sqrt((double) (dx*dx+dy*dy+MagickEpsilon));
    if (length == 0.0)
      {
        n++;
        if (draw_info->dash_pattern[n] == 0.0)
...
#define RenderImageTag  "Render/Image"
  ...
  */
  ...
  primitive_info=(PrimitiveInfo *)
    AcquireMagickMemory((size_t) number_points*sizeof(*primitive_info));
                ...
                if (status == MagickFalse)
                  {
                    ...
                    (void) CopyMagickString(clone_info->filename,token,
                      MaxTextExtent);
                    graphic_context[n]->fill_pattern=
                      ReadImage(clone_info,&image->exception);
    ...
    */
    ...
    switch (primitive_type)
    {
      ...
      case EllipsePrimitive:
      {
        ...
        radius=sqrt((double) (alpha*alpha+beta*beta));
  ...
  const DrawInfo *draw_info,const char *name,Image **pattern)
{
  ...
  (void) FormatMagickString(attribute,MaxTextExtent,"[%s]",name);
  path=GetImageAttribute(image,attribute);
  if (path == (ImageAttribute *) NULL)
    return(MagickFalse);
  (void) FormatMagickString(attribute,MaxTextExtent,"[%s-geometry]",name);
  ...
  image_info->size=AcquireString(geometry->value);
  *pattern=AllocateImage(image_info);
  image_info=DestroyImageInfo(image_info);
  (void) QueryColorDatabase("none",&(*pattern)->background_color,
    &image->exception);
  SetImageBackgroundColor(*pattern);
  if (image->debug != MagickFalse)
    (void) LogMagickEvent(DrawEvent,GetMagickModule(),
      "begin pattern-path %s %s",name,geometry->value);
  clone_info=CloneDrawInfo((ImageInfo *) NULL,draw_info);
  ...
  MagickRealType *stroke_opacity)
{
  ...
  *stroke_opacity=0.0;
  ...
  for (j=0; j < (long) polygon_info->number_edges; j++)
  {
    ...
    for (i=(long) Max(p->highwater,1); i < (long) p->number_points; i++)
    {
  ...
  */
  ...
  if (fill_rule != NonZeroRule)
    {
      if ((AbsoluteValue(winding_number) & 0x01) != 0)
        return(1.0);
    }
  else
  ...
  const DrawInfo *draw_info,const PrimitiveInfo *primitive_info)
{
  ...
  switch (primitive_info->primitive)
  {
    ...
    case ImagePrimitive:
    {
      ...
      if (((unsigned long) (primitive_info[1].point.x+0.5) != composite_image->columns) &&
          ((unsigned long) (primitive_info[1].point.y+0.5) != composite_image->rows))
        {
          char
            geometry[MaxTextExtent];
  ...
  const MagickBooleanType large_arc,const MagickBooleanType sweep)
{
  ...
  if ((theta < 0.0) && (sweep != MagickFalse))
    ...
    if ((theta > 0.0) && (sweep == MagickFalse))
      theta-=(MagickRealType) (2.0*MagickPI);
  arc_segments=(unsigned long)
    ceil(fabs((double) (theta/(0.5*MagickPI+MagickEpsilon))));
  p=primitive_info;
  ...
  const unsigned long number_vertices,const MagickRealType offset)
{
  ...
  distance=sqrt((double) (dx*dx+dy*dy+MagickEpsilon));
    ...
    if (dot_product <= 0.0)
      switch (draw_info->linejoin)
      {
        ...
        case RoundJoin:
        {
          ...
          arc_segments=(unsigned long)
            ceil((double) ((theta.q-theta.p)/(2.0*sqrt((double) (1.0/mid)))));
      ...
      switch (draw_info->linejoin)
      {
        ...
        case RoundJoin:
        {
          ...
          arc_segments=(unsigned long)
            ceil((double) ((theta.p-theta.q)/(2.0*sqrt((double) (1.0/mid)))));
          path_p[p++]=box_p[1];
          for (j=1; j < arc_segments; j++)
          {
            delta_theta=(MagickRealType) (j*(theta.q-theta.p)/arc_segments);
