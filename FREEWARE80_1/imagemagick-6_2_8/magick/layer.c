...
MagickExport Image *CoalesceImages(Image *image,ExceptionInfo *exception)
{
  ...
  */
  ...
  SetImageBackgroundColor(coalesce_image);
  ...
  for ( ; next != (Image *) NULL; next=GetNextImageInList(next))
  {
    ...
    */
    ...
    if ((bounds.x+bounds.width) > coalesce_image->columns)
    ...
    if ((bounds.y+bounds.height) > coalesce_image->rows)
    ...
    */
    ...
    (void) CloneImageAttributes(coalesce_image,next);
...
MagickExport Image *DisposeImages(Image *image,ExceptionInfo *exception)
{
  ...
  */
  ...
  SetImageBackgroundColor(previous_image);
  ...
  for (next=image; next != (Image *) NULL; next=GetNextImageInList(next))
  {
    ...
    if (next->dispose == BackgroundDispose)
      {
        ...
        if ((bounds.x+bounds.width) > current->columns)
        ...
        if ((bounds.y+bounds.height) > current->rows)
    ...
    */
    { Image *dispose=CloneImage(current,0,0,MagickTrue,exception);
      ...
      (void) CloneImageAttributes(dispose,next);
...
//    p=AcquireImagePixels(image1,bounds.x,y,bounds.width,1,exception);
//    q=AcquireImagePixels(image2,bounds.x,y,bounds.width,1,exception);
    if ((p == (const PixelPacket *) NULL) ||
        (q == (const PixelPacket *) NULL))
      break;
    indexes1=GetIndexes(image1);
  ...
  const MagickLayerMethod method, ExceptionInfo *exception)
{
  ...
  */
  ...
  SetImageBackgroundColor(image_a);
  ...
  const MagickLayerMethod method, ExceptionInfo *exception)
{
  ...
  */
  ...
  SetImageBackgroundColor(prev_image);
  /*
