...
static void
  ...
  HilbertCurve(CubeInfo *,const Image *image,ViewInfo *,const unsigned long,
    const unsigned int),
  PruneLevel(CubeInfo *,const NodeInfo *),
  PruneToCubeDepth(CubeInfo *,const NodeInfo *),
  ReduceImageColors(CubeInfo *,const Image *,const unsigned long,
    ExceptionInfo *);

/*
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%
%      MagickBooleanType AssignImageColors(CubeInfo *cube_info,Image *image)
%
  ...
  unsigned long index)
{
  ...
  id=(unsigned long) (((ScaleQuantumToChar(pixel->red) >> index) & 0x01) << 2 |
    ((ScaleQuantumToChar(pixel->green) >> index) & 0x01) << 1 |
    ((ScaleQuantumToChar(pixel->blue) >> index) & 0x01));
  return(id);
}

static MagickBooleanType AssignImageColors(CubeInfo *cube_info,Image *image)
{
        ...
        for (x=0; x < (long) image->columns; x+=count)
        {
          ...
          */
          for (count=1; (x+count) < (long) image->columns; count++)
            if (IsColorEqual(q,q+count) == MagickFalse)
              break;
          node_info=cube_info->root;
          for (index=MaxTreeDepth-1; (long) index > 0; index--)
          {
            id=ColorToNodeId(q,index);
          ...
          */
          cube_info->color.red=(double) q->red;
          cube_info->color.green=(double) q->green;
          cube_info->color.blue=(double) q->blue;
          cube_info->distance=3.0*((MagickRealType) QuantumRange+1.0)*
  ...
  if ((cube_info->quantize_info.number_colors == 2) &&
      (cube_info->quantize_info.colorspace == GRAYColorspace))
    {
      ...
      */
      ...
      {
        intensity=(Quantum)
          (PixelIntensityToQuantum(q) < (QuantumRange/2) ? 0 : QuantumRange);
        q->red=intensity;
        q->green=intensity;
        q->blue=intensity;
        q++;
  ...
  */
  ...
  for (y=0; (y < (long) image->rows) && (cube_info->colors < 256); y++)
  {
    ...
    for (x=0; x < (long) image->columns; x+=count)
    {
      ...
      */
      for (count=1; (x+count) < (long) image->columns; count++)
        if (IsColorEqual(p,p+count) == MagickFalse)
          break;
      index=MaxTreeDepth-1;
      bisect=((MagickRealType) QuantumRange+1.0)/2.0;
      mid.red=(double) QuantumRange/2.0;
      mid.green=(double) QuantumRange/2.0;
      mid.blue=(double) QuantumRange/2.0;
      node_info=cube_info->root;
      for (level=1; level <= MaxTreeDepth; level++)
      {
        bisect*=0.5;
        id=ColorToNodeId(p,index);
        mid.red+=(id & 4) != 0 ? bisect : -bisect;
        mid.green+=(id & 2) != 0 ? bisect : -bisect;
        mid.blue+=(id & 1) != 0 ? bisect : -bisect;
        if (node_info->child[id] == (NodeInfo *) NULL)
          {
            /*
        ...
        */
        node_info=node_info->child[id];
        pixel.red=QuantumScale*(p->red-mid.red);
        pixel.green=QuantumScale*(p->green-mid.green);
        pixel.blue=QuantumScale*(p->blue-mid.blue);
        node_info->quantize_error+=count*sqrt((double) (pixel.red*pixel.red+
          pixel.green*pixel.green+pixel.blue*pixel.blue));
      ...
      */
      ...
      node_info->total_red+=QuantumScale*((MagickRealType) count*p->red);
      node_info->total_green+=QuantumScale*((MagickRealType) count*p->green);
      node_info->total_blue+=QuantumScale*((MagickRealType) count*p->blue);
      p+=count;
    }
    if ((image->progress_monitor != (MagickProgressMonitor) NULL) &&
        (QuantumTick(y,image->rows) != MagickFalse))
  ...
  if (node_info->number_unique != 0)
    {
      ...
      */
      number_unique=(double) ((MagickOffsetType) node_info->number_unique);
      image->colormap[image->colors].red=(Quantum)
        (QuantumRange*node_info->total_red/number_unique+0.5);
      image->colormap[image->colors].green=(Quantum)
        (QuantumRange*node_info->total_green/number_unique+0.5);
      image->colormap[image->colors].blue=(Quantum)
        (QuantumRange*node_info->total_blue/number_unique+0.5);
...
static void DestroyCubeInfo(CubeInfo *cube_info)
{
  ...
  */
  do
  {
    ...
    cube_info->node_queue->nodes=(NodeInfo *)
      RelinquishMagickMemory(cube_info->node_queue->nodes);
...
%
%      MagickBooleanType Dither(CubeInfo *cube_info,const Image *image,
%        ViewInfo *image_view,const unsigned int direction)
%
%  A description of each parameter follows.
%
%    o cube_info: A pointer to the Cube structure.
...
%
*/
static MagickBooleanType Dither(CubeInfo *cube_info,const Image *image,
  ViewInfo *image_view,const unsigned int direction)
{
      ...
      (p->y >= 0) && (p->y < (long) image->rows))
    {
      ...
      */
      ...
      if (q == (PixelPacket *) NULL)
        return(MagickFalse);
      indexes=GetCacheViewIndexes(image_view);
      error.red=(double) q->red;
      error.green=(double) q->green;
      error.blue=(double) q->blue;
      for (i=0; i < ExceptionQueueLength; i++)
      {
        error.red+=p->weights[i]*p->error[i].red;
        error.green+=p->weights[i]*p->error[i].green;
        error.blue+=p->weights[i]*p->error[i].blue;
      }
      pixel.red=RoundToQuantum(error.red);
      pixel.green=RoundToQuantum(error.green);
      pixel.blue=RoundToQuantum(error.blue);
      i=(long) ((pixel.blue >> CacheShift) << 12 |
        (pixel.green >> CacheShift) << 6 | (pixel.red >> CacheShift));
      if (p->cache[i] < 0)
        {
          register NodeInfo
            *node_info;
          ...
          */
          ...
          /*
            Find closest color among siblings and their children.
          */
          p->color.red=(double) pixel.red;
          p->color.green=(double) pixel.green;
          p->color.blue=(double) pixel.blue;
          p->distance=3.0*((MagickRealType) QuantumRange+1.0)*
            ((MagickRealType) QuantumRange+1.0);
...
%
%      MagickBooleanType DitherImage(CubeInfo *cube_info,Image *image)
%
%  A description of each parameter follows.
%
%    o cube_info: A pointer to the Cube structure.
...
%
*/
static MagickBooleanType DitherImage(CubeInfo *cube_info,Image *image)
{
  register unsigned long
    i;
  ...
  */
  ...
  image_view=OpenCacheView(image);
  HilbertCurve(cube_info,image,image_view,depth-1,NorthGravity);
  (void) Dither(cube_info,image,image_view,ForgetGravity);
  image_view=CloseCacheView(image_view);
  return(MagickTrue);
}

  ...
  unsigned long depth)
{
  ...
  */
  cube_info->cache=(long *) AcquireMagickMemory((size_t) (1L << 18)*
  ...
  if (cube_info->free_nodes == 0)
    {
      ...
      */
      ...
      nodes->nodes=(NodeInfo *)
        AcquireMagickMemory(NodesInAList*sizeof(*nodes->nodes));
...
MagickExport MagickBooleanType GetImageQuantizeError(Image *image)
{
  ...
  for (y=0; y < (long) image->rows; y++)
  {
    ...
    {
      distance=fabs(p->red-(double) image->colormap[indexes[x]].red);
      mean_error_per_pixel+=distance;
      mean_error+=distance*distance;
      if (distance > maximum_error)
        maximum_error=distance;
      distance=fabs(p->green-(double) image->colormap[indexes[x]].green);
      mean_error_per_pixel+=distance;
      mean_error+=distance*distance;
      if (distance > maximum_error)
        maximum_error=distance;
      distance=fabs(p->blue-(double) image->colormap[indexes[x]].blue);
      mean_error_per_pixel+=distance;
      mean_error+=distance*distance;
      if (distance > maximum_error)
        maximum_error=distance;
...
%
%      void HilbertCurve(CubeInfo *cube_info,const Image *image,
%        ViewInfo *image_view,const unsigned long level,
...
%
*/
static void HilbertCurve(CubeInfo *cube_info,const Image *image,
  ViewInfo *image_view,const unsigned long level,const unsigned int direction)
{
  if (level == 1)
    {
      switch (direction)
      {
        case WestGravity:
        {
          (void) Dither(cube_info,image,image_view,EastGravity);
          (void) Dither(cube_info,image,image_view,SouthGravity);
          (void) Dither(cube_info,image,image_view,WestGravity);
          break;
        }
        case EastGravity:
        {
          (void) Dither(cube_info,image,image_view,WestGravity);
          (void) Dither(cube_info,image,image_view,NorthGravity);
          (void) Dither(cube_info,image,image_view,EastGravity);
          break;
        }
        case NorthGravity:
  ...
  switch (direction)
  {
    case WestGravity:
    {
      HilbertCurve(cube_info,image,image_view,level-1,NorthGravity);
      (void) Dither(cube_info,image,image_view,EastGravity);
      HilbertCurve(cube_info,image,image_view,level-1,WestGravity);
      (void) Dither(cube_info,image,image_view,SouthGravity);
      HilbertCurve(cube_info,image,image_view,level-1,WestGravity);
      (void) Dither(cube_info,image,image_view,WestGravity);
      HilbertCurve(cube_info,image,image_view,level-1,SouthGravity);
      break;
    }
    case EastGravity:
    {
      HilbertCurve(cube_info,image,image_view,level-1,SouthGravity);
      (void) Dither(cube_info,image,image_view,WestGravity);
      HilbertCurve(cube_info,image,image_view,level-1,EastGravity);
      (void) Dither(cube_info,image,image_view,NorthGravity);
      HilbertCurve(cube_info,image,image_view,level-1,EastGravity);
      (void) Dither(cube_info,image,image_view,EastGravity);
      HilbertCurve(cube_info,image,image_view,level-1,NorthGravity);
...
MagickExport MagickBooleanType OrderedDitherImage(Image *image)
{
  ...
  */
  ...
  for (y=0; y < (long) image->rows; y++)
  {
    ...
    for (x=0; x < (long) image->columns; x++)
    {
      index=(Quantum) (PixelIntensityToQuantum(q) > ScaleCharToQuantum(
        (unsigned char) (DitherMatrix[y & 0x07][x & 0x07] != 0 ? 1 : 0)));
      indexes[x]=index;
      q->red=image->colormap[index].red;
      q->green=image->colormap[index].green;
      q->blue=image->colormap[index].blue;
  ...
  const unsigned long levels,const MagickBooleanType dither)
{
  ...
  */
  ...
  for (l=2; (l*l*l) <= (long) posterize_image->colors; l++);
  l--;
  ...
  for (i=0; i < l; i++)
    for (j=0; j < l; j++)
      for (k=0; k < l; k++)
      {
        posterize_image->colormap[n].red=(Quantum) (QuantumRange*i/(l-1));
        posterize_image->colormap[n].green=(Quantum) (QuantumRange*j/(l-1));
        posterize_image->colormap[n].blue=(Quantum) (QuantumRange*k/(l-1));
  ...
  if (status != MagickFalse)
    {
      ...
      */
      ReduceImageColors(cube_info,image,number_colors,&image->exception);
      status=AssignImageColors(cube_info,image);
    }
  if (quantize_info->colorspace != RGBColorspace)
    (void) SetImageColorspace(image,quantize_info->colorspace);
  DestroyCubeInfo(cube_info);
  return(status);
}

  ...
  if (depth == 0)
    {
      ...
      */
      ...
      if (quantize_info->dither != MagickFalse)
        depth--;
      pseudo_class=MagickTrue;
      image=images;
      for ( ; image != (Image *) NULL; image=GetNextImageInList(image))
        if (image->storage_class != PseudoClass)
          pseudo_class=MagickFalse;
      if (pseudo_class != MagickFalse)
        depth+=2;
    }
  /*
    Initialize color cube.
...
%
%      ReduceImageColors(CubeInfo *cube_info,const Image *image,
%        const MagickBooleanType number_colors,ExceptionInfo *exception)
%
%  A description of each parameter follows.
%
%    o image: The image.
...
%
*/
static void ReduceImageColors(CubeInfo *cube_info,const Image *image,
  const unsigned long number_colors,ExceptionInfo *magick_unused(exception))
{
#define ReduceImageTag  "Reduce/Image"
  ...
  while (cube_info->colors > number_colors)
  {
    ...
    if ((image->progress_monitor != (MagickProgressMonitor) NULL) &&
        (QuantumTick(offset,span-number_colors+1) != MagickFalse))
      {
        status=image->progress_monitor(ReduceImageTag,offset,span-
          number_colors+1,image->client_data);
        if (status == MagickFalse)
          break;
      }
  }
