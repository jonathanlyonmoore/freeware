  ...
  Image *image)
{
  ...
  */
  ...
  {
    if (icon_width >= (unsigned int) (scale_factor*width))
      break;
  ...
  {
    if (icon_height >= (unsigned int) (scale_factor*height))
      break;
...
#define XVisualColormapSize(visual_info) Min((unsigned int) (\
  (visual_info->klass == TrueColor) || (visual_info->klass == DirectColor) ? \
   ...
   (unsigned int) visual_info->colormap_size),1U << (unsigned int) visual_info->depth)
  ...
  const XResourceInfo *resource_info,Image *image,XPixelInfo *pixel)
{
  ...
  */
  ...
  packets=(unsigned int)
    Max((int) pixel->colors,visual_info->colormap_size)+MaxNumberPens;
  if (pixel->pixels != (unsigned long *) NULL)
    pixel->pixels=(unsigned long *) RelinquishMagickMemory(pixel->pixels);
  ...
  XResourceInfo *resource_info)
{
  ...
  */
  ...
  if (resource_info->gravity == (-1))
    {
      ThrowXWindowException(OptionError,"UnrecognizedGravityType",
        resource_value);
      resource_info->gravity=CenterGravity;
    }
          ...
          (height != (unsigned int) window->image->rows))
        {
          ...
          */
          ...
          if ((window->pixel_info->colors != 0) ||
              (window->image->columns > display_width) ||
              (window->image->rows > display_height))
